function solve_steady_beam_contact_vectorF()
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    M         = 128;              % Number of intervals (=> M+1 points)
    ...
    % yline(y_minus, 'k--', 'LineWidth',1.2, 'DisplayName','Obstacle');
    for idx = 1:length(f_values)
        plot(xvals, all_solutions(:, idx), '-', 'Color', cm(idx,:), ...
             'DisplayName',['f = ', num2str(f_values(idx))],'LineWidth',1.2);
    end
    ...
    title('Numerical Solution for the Steady-State Problem with Constant Forces',Interpreter='latex');
