...
A=5*2; % m^265
rho=2700; % kg/m
E=10e9; % Pa . %%%% 85 for beta 0.01 and 10 for beta 0.1, 300 for 1 and 10
I=0.2*(0.05^3)/12; % m^4
...
plot(t_sub, dist0M_sub, ...
     ...
     'LineWidth', 0.1, ...        % thicker line for clarity
...
% 
% fig_fft = figure;
% plot(abs(uFFT),'LineWidth', 0.5,'MarkerSize', 1.5);
% xlabel('Frequency (Hz)');
% ylabel('|FFT[u(M,t)]|');
...
% 
% % Build a filename using the current kappa value
% filename_fft = sprintf('ext_dnc_fft_%.2g.jpg', kappa);
...
% 
% % Build a filename using the current kappa value
% filename_fftshift = sprintf('ext_dnc_fftshift_%.2g.jpg', kappa);
% % saveas(fig_fftshift, filename_fftshift);  % Save the figure


% %% -- 1) Define your time-domain signal and sampling rate --
% y    = dist0M;   % tip displacement (already computed in your code)
% Fs   = 1/length(t_sub);     % sampling frequency
% N    = length(y);% number of samples
% 
% %% -- 2) Take the FFT of the signal --
% Y = fft(y);      % raw FFT output (complex values)
% 
% %% -- 3) Convert to single-sided amplitude spectrum --
% P2 = abs(Y/N);           % two-sided spectrum, normalized by N
% P1 = P2(1:floor(N/2)+1); % single-sided spectrum
% P1(2:end-1) = 2*P1(2:end-1);  % double amplitudes for indices 2..end-1
% 
% %% -- 4) Construct the frequency axis (Hz) --
% f = Fs*(0:floor(N/2))/N;
% 
% %% -- 5) Plot the frequency vs. amplitude --
% figure;
% plot(f,P1,'LineWidth',1.5)
% xlabel('Frequency (Hz)')
% ylabel('Amplitude')
% title('Single-Sided Amplitude Spectrum of the Beam Tip Displacement')
% grid on;
