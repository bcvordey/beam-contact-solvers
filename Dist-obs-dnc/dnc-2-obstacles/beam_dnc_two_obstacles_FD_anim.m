 function beam_dnc_two_obstacles_FD_anim()
...
% (delta = 1). Plots u(1,t) for multiple beta values and animates u(x,t).
    ...
    %% ---------------- Animation ----------------
    fig1 = figure('Name', sprintf('Beam with DNC: animation (\\beta = %g)', beta_anim), ...
                  'Color','w');
    ...
    title('Vibration with Two-Sided Damped Normal Compliance at x=L');
    axis([0, x(end), -ymax, ymax]);

    % --- Animation pacing controls ---
    skip = 2;                 % show every 'skip' frames
    playback_speed = 100;       % sim-seconds per real-second (larger = faster playback)
    ...
    for j = 1:skip:numel(t)
        ...
        drawnow;
        pause( (skip*dt)/max(playback_speed, eps) );
    end
