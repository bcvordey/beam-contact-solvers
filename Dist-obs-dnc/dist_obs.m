...
alpha2  = 1.296;      % alpha^2
kappa   = 0.1;    % obstacle stiffness
f0      = -0.25;   % constant forcing f(x,t)=f0
betaVec = 1;  % one or more beta values

% Obstacle profile psi(x)
% psiFun  = @(x) -0.02 + 0*x;
psiFun = @(x) -0.02 + 0.01*sin(pi*x).^2;
...
hTitle = title(ax, sprintf('Distributed Obstacle at $t = %.2f$  ($\\kappa = %.3g, \\beta = %.3g$)', ...
    t(1), kappa, S.beta),'Interpreter','latex');
