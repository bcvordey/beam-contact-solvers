...
kappaVec = [0.1, 10, 1e2, 1e3, 1e4, 1e5];
betaVec  = 0.1;
forceVec = -1.65;
...
timeIntervalAdjust = 2500;    % for subsampling time
...
for kk = 1:length(kappaVec)
    for bb = 1:length(betaVec)
        for ff = 1:length(forceVec)
            ...
            xlabel('Frequency (Hz)');
            ylabel('|FFT[u(M,t)]|');
            title({'FFT of Contact End of the Beam', sprintf('\\kappa=%.2g', kappa)});
            ...
            fig_fftshift = figure;
            plot(abs(uFFTshift));
            title({'FFT SHIFT of Contact End of the Beam', sprintf('\\kappa=%.2g', kappa)});
            xlabel('Frequency (Hz)');
